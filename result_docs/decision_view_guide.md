# Decision View Guide — RMEDA Dashboard
...
* **Template**: [decision_view.html](file:///c:/Smeet_internTask/analysisWork3/templates/decision_view.html)
* **Controller**: `decision_view()` in [app.py](file:///c:/Smeet_internTask/analysisWork3/app.py)

---

## 1. Phase 1: The 7 Structured Business Cards

The 7 business cards present an executive-level view of the data drift. The list of cards is built by the function `build_business_insights(results, stage)` inside [abt/business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py).
...
* **Logic**: Uses [abt/signal_collector.py](file:///c:/Smeet_internTask/analysisWork3/abt/signal_collector.py) to compile metrics (PSI, boundary drift, variance shift). It determines the exact mechanism (e.g., center shift, spread change) and creates a headline, evidence, and suggested action.
* **Backend Function**: `_top_drift_insights()` in [business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py)
...
* **Backend Function**: `_insight_target()` in [business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py)
...
* **Backend Function**: `_insight_pipeline()` in [business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py)
...
* **Backend Function**: `_insight_model_risk()` in [business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py)
...
* **Backend Function**: `_insight_governance()` in [business_insights.py](file:///c:/Smeet_internTask/analysisWork3/abt/business_insights.py)
...
1. **Call 1: Drift Triage (Triage)**
   * **Prompt File**: [abt/llm_drift_narratives.py](file:///c:/Smeet_internTask/analysisWork3/abt/llm_drift_narratives.py) (`_triage_prompt`)
   * **Action**: Filters the drifted columns down to the most important ones.
2. **Call 2: Narrative Generation (Write-up)**
   * **Prompt File**: [abt/llm_drift_narratives.py](file:///c:/Smeet_internTask/analysisWork3/abt/llm_drift_narratives.py) (`_writeup_prompt`)
...
3. **Call 3: Portfolio Synthesis (Executive Summary)**
   * **Prompt File**: [abt/llm_drift_narratives.py](file:///c:/Smeet_internTask/analysisWork3/abt/llm_drift_narratives.py) (`_meta_prompt`)
   * **Action**: Generates a single-sentence overview of the entire dataset shift.

* **Backend Function**: `synthesise_drift_insights_v2()` in [abt/llm_drift_narratives.py](file:///c:/Smeet_internTask/analysisWork3/abt/llm_drift_narratives.py) (called during `run_comparison()`)

---

## 3. The Validation Layer (`insight_validator.py`)

To ensure the actions recommended on the 7 business cards are accurate and comply with corporate risk standards, they are processed by **[`abt/insight_validator.py`](file:///c:/Smeet_internTask/analysisWork3/abt/insight_validator.py)**.
...
* **Backend Function**: `validate_insights()` in [abt/insight_validator.py](file:///c:/Smeet_internTask/analysisWork3/abt/insight_validator.py)
