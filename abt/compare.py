...
def run_comparison(abts: List[ABTProfile], use_llm: bool = True, cfg=None) -> Dict:
    ...
    # ── Tier A: original rule-based interpretations (i1–i9) ──────────────────
    try:
        from .interpretations import run_interpretations
        results = run_interpretations(results)
    except Exception:
        pass  # always additive — never breaks the page

    # ── Tier C: hybrid interpretations with Tier A + Tier B logic onlu ; no llm (i4b–i9b) ─
    try:
        from .interpretations import run_interpretations_hybrid
        results = run_interpretations_hybrid(results)
    except Exception:
        pass

    # ── Tier B: enhanced interpretations with LLM narrative fields (i4b–i9b) ─
    try:
        from .interpretations import run_interpretations_enhanced
        results = run_interpretations_enhanced(results)
    except Exception:
        pass

    

    # # ── LLM enrichment: fills narrative keys on i4b–i9b and c0/c8 ────────────
    # if use_llm:
    #     try:
    #         from .llm_insights import enrich_compare
    #         results = enrich_compare(results)
    ...
    # Business insights layer — always runs, always optional
    try:
